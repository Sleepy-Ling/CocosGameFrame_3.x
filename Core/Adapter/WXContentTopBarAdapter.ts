...
export default class WXGameTopBarUIAdapter extends Component {
    start() {
        if (WECHAT) {
            ...
            let paddingTop = parentTransform.height * (menuInfo.top / systemInfo.screenHeight);

            // 原理同上，我只是将本节点改为和微信菜单栏一样的高度
            uiTransform.height = parentTransform.height * (menuInfo.height / systemInfo.screenHeight);
