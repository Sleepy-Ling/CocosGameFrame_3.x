...
import { Component, Node, screen, UITransform, view, Widget, _decorator } from "cc";
...
export default class ContentAdapter extends Component {
    onLoad() {
        ...
        let srcScaleForShowAll = Math.min(screen.windowSize.width / uiTransform.width, screen.windowSize.height / uiTransform.height);
        let realWidth = uiTransform.width * srcScaleForShowAll;
        let realHeight = uiTransform.height * srcScaleForShowAll;

        // 2. 基于第一步的数据，再做节点宽高适配
        uiTransform.width = uiTransform.width * (screen.windowSize.width / realWidth);
        uiTransform.height = uiTransform.height * (screen.windowSize.height / realHeight);
